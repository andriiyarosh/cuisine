...
class ReceiptViewModel @Inject constructor(private val receiptsRepository: ReceiptsRepository) :
    BaseReceiptViewModel() {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            _receipts.postValue(
                RequestSteps.RequestFinished(
                    receiptsRepository.getReceipt(query, cuisine.value, number)?.apply {
                        val uri = baseUri
